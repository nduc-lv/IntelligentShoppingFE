...
import { ArrowLeft, ChevronRight, Info, Plus, Trash2 } from "lucide-react-native";
import moment, { Moment } from "moment";
import React, { useState, useEffect } from "react";
import { View, Text, StyleSheet, ActivityIndicator, FlatList, TouchableOpacity, Image, Modal, TextInput, Button, ScrollView } from "react-native";
import { RootScreens } from "..";
import { useCreateMenuMutation, useDeleteMenuMutation, useLazyGetMenuQuery } from "@/Services/menu";
...
export const MenuCalendarScreen = () => {
    const route = useRoute<RouteProp<MenuCalendarRouteParams, "MenuCalendarInfo">>();
    const { groupId, isAdmin } = route.params;
    const [currentWeekStart, setCurrentWeekStart] = useState(moment().startOf('week')); // Ngày bắt đầu của tuần hiện tại
    ...
    const handleCreateMenu = async () => {
        ...
        if (hasDuplicatesOrEmpty(recipeList)) {
            toast.show("Danh sách món ăn không hợp lệ", { placement: "top", type: "warning" });
            return;
        }
        try {
            const payload = {
            ...
            await getMenu({ group_id: groupId, date: selectedDate.toISOString() }).unwrap();
            handleCloseActionSheet();
            toast.show("Tạo thực đơn thành công", { placement: "top", type: "success" });
        } catch (e) {
            console.log(e)
            toast.show("Không thể tạo thực đơn", { placement: "top", type: "warning" });
        }
    }
    ...
    const handleDeleteMenu = async (menu_id: string) => {
        try {
            ...
            await getMenu({ group_id: groupId, date: selectedDate.toISOString() }).unwrap();
            toast.show("Xóa thực đơn thành công", { placement: "top", type: "success" });
        } catch (e) {
            console.log(e)
            toast.show("Không thể xóa thực đơn", { placement: "top", type: "warning" });
        }
    }
        ...
        <View style={styles.eventCard}>
            ...
            <View style={styles.recipes}>
                <Text style={styles.recipeNames}>
                    {item.menu_recipe.map((menu_recipe: any) => menu_recipe.recipe.name).join(', ')}
                </Text>
            </View>
    ...
    const renderRecipeList = ({ item, index }: { item: any, index: number }) => (
        <View style={styles.eventCard}>
            ...
            <TouchableOpacity onPress={() => removeSelector(index)} style={styles.trashIcon}>
    ...
    const recipeOptions = recipes && recipes.map((recipe: any) => ({
    ...
    return (
        <View style={styles.container}>
            ...
            {isAdmin &&
                <TouchableOpacity style={styles.fab} onPress={() => handleOpenActionSheet()}>
                    <Plus color="white" size={25} />
                </TouchableOpacity>}
            {isOpenCreateActionSheet &&
                <Actionsheet isOpen={isOpenCreateActionSheet}
